"""Hybrid keyword/BM25/HRR retrieval for the memory store.

Ported from Hermes Agent (plugins/memory/holographic/retrieval.py).

Combines three retrieval strategies:
  1. FTS5 full-text search (BM25-style ranking)
  2. Jaccard token similarity (reranking)
  3. HRR vector similarity (compositional/structural matching)

Plus three algebraic query types that no embedding DB can do:
  - probe(entity): unbind entity from memory bank to extract associated content
  - related(entity): find facts structurally connected to an entity
  - reason(entities): multi-entity compositional JOIN (AND semantics)
  - contradict(): find potentially contradictory facts via entity overlap
                  + content divergence
"""
...
class FactRetriever:
    ...
    def search(
        ...
        3. HRR similarity: Phase vector cosine similarity
        ...
        query_vec = None  # lazy — encode once on first candidate with HRR vector
        scored = []
    ...
    def related(
        ...
        facts connected through shared context — other entities mentioned
        alongside this one, or content that overlaps structurally.
        """
        if not hrr._HAS_NUMPY:
            return self.search(entity, category=category, limit=limit)
    ...
    def reason(
        ...
        AND semantics via min() — a fact scores high only if ALL entities
        have structural presence.
    ...
    def _fts_candidates(
        ...
        """Get raw FTS5 candidates. Normalizes FTS5 rank to [0, 1]."""
        conn = self.store._conn
        params: list = []
    ...
    def _tokenize(text: str) -> set[str]:
        """Simple whitespace tokenization with lowercasing. Strips punctuation."""
        if not text:
            return set()
        tokens = set()
    ...
    def _sanitize_fts_query(cls, query: str) -> str:
        ...
        FTS5 AND-joins tokens by default, which kills recall on prose queries.
        This drops stopwords, strips FTS5 special chars, and OR-joins survivors.
        """
        if not query:
            return ""
        _FTS_SPECIAL = '"()*^:-+'
    ...
    def _temporal_decay(self, timestamp_str: str | None) -> float:
        """Exponential decay: 0.5^(age_days / half_life_days). Returns 1.0 if disabled."""
