...
Ported from Hermes Agent (plugins/memory/holographic/store.py), adapted for Kyourai.

Schema:
  - facts: content, category, tags, trust_score, retrieval/helpful counts, hrr_vector
  - entities: name, type, aliases
  - fact_entities: many-to-many link
  - facts_fts: FTS5 virtual table over facts(content, tags)
  - memory_banks: per-category bundled HRR vectors for compositional queries

Trust scoring: asymmetric feedback (helpful +0.05, unhelpful -0.10) clamped
to [0, 1]. Trust multiplies retrieval scores so reliable facts rank higher.
"""
...
def _apply_wal_with_fallback(conn: sqlite3.Connection, db_label: str = "memory_store.db") -> None:
    """Enable WAL mode, falling back to journal mode on filesystems that don't support it."""
    try:
...
class MemoryStore:
    ...
    Uses a process-wide shared connection registry so all instances pointing
    at the same database file share ONE connection and ONE lock — preventing
    cross-connection write contention.
    """
    ...
    def __init__(
        ...
        if db_path is None:
            ...
            db_path = str(get_kyourai_home() / "memory_store.db")
        ...
        with MemoryStore._shared_guard:
            ...
            if entry is None:
                conn = sqlite3.connect(
                    ...
                    timeout=10.0,
                    isolation_level=None,  # autocommit — no dangling transactions
                )
    ...
    def _init_db(self) -> None:
        """Create tables, indexes, triggers. Enable WAL mode."""
    ...
    def search_facts(
        ...
        """FTS5 keyword search. Increments retrieval_count for returned facts."""
    ...
    def _extract_entities(self, text: str) -> list[str]:
        """Extract entity candidates using regex rules. Deduplicated, first-seen order."""
        seen: set[str] = set()
        candidates: list[str] = []
    ...
    def _resolve_entity(self, name: str) -> int:
        """Find existing entity by name/alias (case-insensitive) or create one."""
        row = self._conn.execute(
    ...
    def close(self) -> None:
        """Release this instance's refcount on the shared connection."""
        with MemoryStore._shared_guard:
            entry = self._entry
            entry["refs"] -= 1
            if entry["refs"] <= 0:
