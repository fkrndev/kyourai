...
Ported from Hermes Agent (plugins/memory/holographic/holographic.py).
...
def phases_to_bytes(phases: "np.ndarray", dim: int | None = None) -> bytes:
    ...
    that case we fall back to writing raw float64.
    """
    numpy = _np()
    if dim is None:
...
def bytes_to_phases(data: bytes, dim: int | None = None) -> "np.ndarray":
    ...
    promoted to float64 so downstream HRR math keeps the existing behavior.
    """
    numpy = _np()
